...
Phase 3 在上传入库时强校验，并在每次下载前对已安全打开的同一文件句柄重新计算 SHA256，同时核对
文件大小。这样同大小字节篡改也会在返回任何内容前转为 MISSING 并写完整性失败审计。恢复、MISSING
修复和 reconciliation 继续执行真实类型、扫描及其他完整安全复核；任何已知缺失或校验失败均禁止下载。
