...
项目级权限需要明确定义“角色默认权限”和“单成员例外权限”的优先级。V1 默认采用：
...
如果实现单成员例外权限，对应字段必须能够区分“未设置、明确允许、明确拒绝”三种状态；不能用一个默认 `False` 的 Boolean 同时表示“继承角色”和“明确拒绝”。若 V1 不需要例外权限，则只实现项目角色默认权限，避免制造表面存在但语义不清的开关。
